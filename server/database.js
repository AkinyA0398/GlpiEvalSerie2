...
import { prepareAssetRefs } from './entityService.js';
  ...
  try {
    ...
    if (syncToGlpi && glpiSessionToken) {
      for (const row of rows) {
        const refs = await prepareAssetRefs({ sessionToken: glpiSessionToken, row });
        ...
        const itemId = await glpiRequest({
          ...
          body: {
            input: {
              ...
              // Some GLPI setups require users_id to be non-null.
              // If the CSV does not contain a user, keep the field out or
              // fallback to a default user if configured.
              users_id: refs.userId || process.env.GLPI_DEFAULT_USER_ID || 0,
            },
          },
        }).then(created => created?.id ?? created?.result?.id ?? created?.['id']);
